...
function fixModelOrders() {
  ...
  for (const brand of brands) {
    ...
    for (const series in modelsBySeries) {
      ...
      // Sort by year desc (newest first), then by name
      models.sort((a, b) => {
      ...
      models.forEach((item, index) => {
        ...
        console.log(`${brand}/${item.file}: order = ${item.data.order} (${item.data.year} - ${item.data.model})`);
      });
    }
  }

  console.log('\n✅ Alle modeller har nu korrekte rækkefølger!');
}
